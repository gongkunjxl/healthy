...
function  cancelTop(obj)
{
    // alert(obj.getAttribute("value"));
    layer = layui.layer;
 	layer.confirm('确认取消置顶该视频？', { title:['取消置顶视频信息提示','font-size:20px; text-align:center']}, function(index)
	{
		layer.close(index);
		window.location.href="/backend/videoCancelTop/"+obj.getAttribute("value");
	});
}
...
function  pushTop(obj)
{
    // alert(obj.getAttribute("value"));
    layer = layui.layer;
 	layer.confirm('确认置顶该视频？', { title:['置顶视频信息提示','font-size:20px; text-align:center']}, function(index)
	{
		layer.close(index);
		...
		$.ajax({
					...
					success: function (data) {
						if(data.count>2){
				     	...
				     	window.location.href="/backend/videoAdmin";
				    },
				    error: function(data) {
